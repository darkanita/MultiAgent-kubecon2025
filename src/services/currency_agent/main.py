"""Currency Agent Microservice - MCP Server over HTTP.

This service exposes the Currency Exchange MCP server over HTTP,
...
from agent.mcp_currency_server import CurrencyMCPServer
...
mcp_server: CurrencyMCPServer | None = None
...
async def lifespan(app: FastAPI):
    ...
    global mcp_server
    logger.info("🚀 [MCP] Starting Currency Agent MCP Server...")
    
    # Initialize MCP server
    mcp_server = CurrencyMCPServer()
    logger.info("✅ [MCP] Currency Agent MCP Server initialized")
    logger.info("🔧 [MCP] Available tools: get_exchange_rate, convert_amount")
    
    yield
    
    logger.info("⏹️  [MCP] Shutting down Currency Agent MCP Server...")


# Create FastAPI app
app = FastAPI(
    title="Currency Agent MCP Server",
    description="Microservice exposing currency exchange tools via MCP over HTTP",
    version="1.0.0",
    lifespan=lifespan
)
...
if __name__ == "__main__":
    ...
    logger.info(f"🌐 Starting Currency Agent on port {port}")
